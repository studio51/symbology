...
# Covers the drawing half of Barcoder (gems/barcoder): the SVG, the PNG, and the
# options both take.
#
...
class BarcoderRenderingTest < Minitest::Test
  ...
  test "an SVG prints every digit, and none when asked not to" do
    ...
    assert_not_includes Barcoder.svg(VALUE, text: false), "<text"
  end

  test "an SVG without text is only as tall as its bars" do
    assert_includes Barcoder.svg(VALUE, text: false, height: 40), %(height="40")
  end

  test "an SVG takes its colours and its font" do
    svg = Barcoder.svg(VALUE, foreground: "#1f2937", font_family: "Inter")
  ...
  test "a transparent SVG has no paper at all" do
    assert_not_includes Barcoder.svg(VALUE, background: nil), "<rect width=\"226\""
    assert_includes Barcoder.svg(VALUE), %(fill="#FFFFFF")
  end

  test "a fractional module width is written out, not rounded" do
    assert_includes Barcoder.svg(VALUE, module_width: 1.5), %(width="169.5")
  end

  # --- PNG -------------------------------------------------------------------

  test "a PNG is a PNG" do
    png = Barcoder.png(VALUE)
  ...
  test "a PNG is the size the geometry says" do
    header = png_header(Barcoder.png(VALUE))
  ...
  test "the modules read back out of the pixels are the modules that went in" do
    pattern = Barcoder.encode(VALUE)
    pixels  = png_pixels(Barcoder.png(VALUE))
  ...
  test "a PNG scales by whole pixels, because half a bar is a different digit" do
    header = png_header(Barcoder.png(VALUE, module_width: 2.4, height: 40.6))
  ...
  test "a PNG carries its colours in a two-entry palette" do
    png = Barcoder.png(VALUE, foreground: "#123", background: "#ffffff")

    assert_equal [ 255, 255, 255, 17, 34, 51 ], png_chunk(png, "PLTE").bytes
  end

  test "a transparent PNG says so, and an opaque one doesn't" do
    assert_equal [ 0 ], png_chunk(Barcoder.png(VALUE, background: nil), "tRNS").bytes
    assert_nil png_chunk(Barcoder.png(VALUE), "tRNS")
  end

  test "a PNG colour has to be a colour" do
    assert_raises(ArgumentError) { Barcoder.png(VALUE, foreground: "rebeccapurple") }
  end

  # --- Options ---------------------------------------------------------------

  test "the quiet zones can be overridden, together or apart" do
    assert_equal 285, png_header(Barcoder.png(VALUE, quiet_zone: 0))[:width]        # the 95-module symbol alone, at 3px
    assert_equal 303, png_header(Barcoder.png(VALUE, quiet_zone: [ 4, 2 ]))[:width] # 101 modules at 3px
  end

  test "an option nobody has is a mistake, not a shrug" do
    error = assert_raises(ArgumentError) { Barcoder.svg(VALUE, colour: "#000") }

    assert_match "unknown option: colour", error.message
  end

  test "a format nobody draws is refused by name" do
    error = assert_raises(Barcoder::UnknownFormat) { Barcoder.render(VALUE, format: :gif) }

    assert_match "svg or png", error.message
  end

  test "an undrawable code is refused before anything is drawn" do
    assert_raises(Barcoder::InvalidCheckDigit) { Barcoder.svg("5901234123456") }
    assert_raises(Barcoder::UnsupportedValue) { Barcoder.png("12345") }
  end

  test "each format knows what to be served as" do
    assert_equal "image/svg+xml", Barcoder.mime_type(:svg)
    assert_equal "image/png", Barcoder.mime_type("png")
  end
  ...
  #
  # Only has to understand what Barcoder writes: one bit a pixel, filter type 0
  # on every row.
  #
  # @param png [String] the image.
  #
